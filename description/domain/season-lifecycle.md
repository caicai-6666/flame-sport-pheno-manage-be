...
当上海业务日期落在赛季起止日期闭区间内、赛季仍为 `0 未开始` 且当前不存在其他 `1 进行中` 赛季时，定时任务自动执行 `0 未开始 → 1 进行中`。

激活与上一赛季结算相互独立：平台允许一个赛季处于 `2 结算中` 的同时，下一个赛季处于 `1 进行中`。任务不会激活结束日期已经过去的未开始赛季；发现多个进行中赛季或多个同时符合条件的候选时停止本轮写入并记录数据一致性错误。
...
- 客户端已提供补传提交能力，但尚无补传期限自动收口；存在非零资格时赛季会持续保持结算中。
