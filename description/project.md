...
项目目前处于基础架构初始化阶段。仓库已经建立 FastAPI 应用入口、Python 基础依赖、异步 MySQL 会话、客户端后端 HTTP 适配器、管理端共享密钥认证和基础测试，但尚未形成完整业务模块、管理员角色权限及完整接口协议。

因此，在相关方案被明确之前：

- 当前应用结构和基础连接方式以 [FastAPI 应用结构与基础连接](infrastructure/application-structure.md) 为准；
- 当前管理端接口使用 [管理员共享密钥与短期 Bearer Token](api/admin-authentication.md) 作为统一访问门槛；除健康检查和 `/auth/login` 登录入口外，管理 API 默认需要有效 token；
- 已确认基础技术依赖包括 FastAPI、Uvicorn、SQLModel、asyncmy、Pydantic Settings、HTTPX、OpenAI SDK 和 Pillow，具体用途以 [Python 运行依赖](infrastructure/python-dependencies.md) 为准；
- 不得自行假定迁移工具、任务调度框架或尚未形成文档的架构细节；
- 不得把共享密钥认证等同于管理员个人身份、角色和权限模型，也不得自行补全细粒度授权规则；
...
积分不是每上传一条凭证就立即发放。赛季进行期间的主要任务是推进项目进度并完成凭证审核；赛季进入“结算中”后，再统一判断用户的赛季完成情况并结算积分。全部结算工作完成后，赛季才标记为“已结束”。
...
积分发放必须同时生成有效积分流水，并确保赛季结算结果与用户全局积分余额一致。重复执行结算任务时，不能重复发放奖励。`point_record.gift_distribution_status` 只描述兑换礼品的发放情况，不参与赛季积分发放。
...
除 [管理端密钥认证 API](api/admin-authentication.md) 已确认的共享密钥与短期 token 机制外，以下事项尚未被当前资料确认，不得视为既定设计：

- 管理员个人账号、身份来源、角色和权限矩阵；
- 管理端 API 的路径、版本、分页、排序和统一响应格式；
- 具体 Web 框架、ORM、依赖注入和任务调度方案；
- 客户端现有后端与管理端后端的部署关系；
- 初审模型服务、文件服务和排行榜任务由哪个进程负责；
- 除管理员 token 进程内缓存外的业务缓存、消息队列、对象存储和部署环境；
...
| 技术栈 | 已确定 Python 3.12、FastAPI 基础结构、异步 MySQL 驱动和 unittest 基础测试 | 数据库版本、迁移工具、完整测试策略及部署方式 |
