...
- 接口先锁定 `season_user`，确保终审不会与用户定分并发覆盖。
- 若目标项目已经由终审通过记录的原始贡献独立达到 `100%`，系统会先自动拒绝该项目全部额外待终审记录并关闭资格；响应返回目标记录的实际拒绝结果，请求中的人工决定不会覆盖它。
- 至少完成一个项目时，为该用户当前全部待终审凭证补齐补传资格。
- 尚未获得补传资格且一个项目都未完成时，终审决定强制使用结算拒绝规则，即使请求传入 `approved`，响应也会返回实际保存的 `rejected`；已有资格的用户不会因后续进度降为零而失去补传权。
- 终审通过会关闭该凭证资格；最后一个阻塞状态消失后，在同一事务内立即写入最终积分和赛季结算通知。
- 任务自动拒绝零完成用户时只创建赛季汇总通知，不重复创建逐凭证拒绝通知。

结算管理页面应使用 [`POST /settlement/final-review`](settlement.md) 提交终审。该接口复用本节的终审核心，但额外强制凭证属于当前结算赛季，并明确承担补传资格更新和自动定分职责。通用接口保留现有结算联动行为，以兼容既有调用方。
...
- [赛季补传资格表说明](../db/season-supplement-eligibility.md)
...
待终审查询在显式只读事务中执行。普通终审拒绝把状态更新、进度撤销、候选回补、项目进度写回和通知创建放在同一个事务中；结算中终审还会在该事务维护资格并尝试定分。用户行锁和项目行锁用于串行化定分与进度分配。通知写入失败时终审事务整体回滚。管理前端读取图片时应把凭证 `id` 传给 [图片安全中转 API](image.md) 的 `/image/proof_record/{proof_record_id}`。
...
测试覆盖待终审查询，以及终审通过不通知、拒绝通知、拒绝回退、终审通过凭证优先回补、回补不足、结算资格关闭、冗余凭证自动拒绝、零完成强制拒绝、重复终审、进度一致性、通知失败回滚、参数校验和认证。
