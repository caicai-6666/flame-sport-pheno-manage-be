...
# 把稳定工具标签放在动态检索 YAML 前，确保 auto 解析并提高跨请求前缀复用率。
def _build_inspection_task_message(
    table_name: str,
    schema_response: TableSchemaToolResponse,
    request: str,
    tool_tag_template: str | None,
) -> dict[str, str]:
    task_content = render_yaml_context(
        {
            "inspection_request": {
                "target_table": table_name,
                "table_schema": parse_yaml_context(schema_response.result),
                "request": request,
            }
        }
    )
    return {
        ...
        "content": build_tool_tag_prefixed_task_content(
            ...
            tool_tag_template,
            (
                "必须调用 execute_single_table_select，不要输出普通文本。"
...
# 在模型未产生工具调用时返回可重试协议错误，并附加所选模型的标签模板。
def _build_missing_inspection_tool_feedback(
    tool_tag_template: str | None,
) -> dict[str, str]:
    error: dict[str, Any] = {
        "code": "inspection_tool_call_missing",
        "message": "本轮没有调用工具，普通文本不能提交单表查询。",
        "repair_action": "只调用 execute_single_table_select，并提交一条受限单表 SELECT SQL。",
    }
    if tool_tag_template is not None:
        error["tool_call_format_guidance"] = {
            "instruction": "严格仿照 template 输出工具标签、工具名和 JSON 参数。",
...
# 将错误工具名或多工具调用作为原 tool_call_id 的失败结果，要求只重试唯一受限工具。
def _build_inspection_protocol_tool_error(
    tool_call_id: str,
    tool_name: str,
) -> dict[str, str]:
    repair_action = "重新生成本轮响应，并且只调用 execute_single_table_select。"
    return {
        "role": "tool",
        ...
        "content": json.dumps(
            {
                "status": "failure",
                "error": {
                    "code": "inspection_invalid_tool_combination",
                    "tool_name": tool_name,
                    "message": "单表检索每轮必须且只能调用 execute_single_table_select。",
                    "details": [
                        {
                            "error_type": "tool_protocol_violation",
                            "field_path": "$",
                            "message": "本轮工具名或工具数量不符合单表检索协议。",
...
class SingleTableDataInspector:
    ...
    # 异步创建一次受限单表检索；名称精确未命中时自动回退候选扫描，避免别名被误判为不存在。
    async def inspect(
        self,
        table_name: str,
        request: str,
        ...
        schema_response = await asyncio.to_thread(self._schema_reader, table_name)
        ...
        messages: list[Any] = [
            ...
            _build_inspection_task_message(
                table_name,
                schema_response,
                request,
        ...
        for attempt_index in range(max_attempt_count):
            ...
            if (
                ...
                if not tool_calls:
                    feedback_message = _build_missing_inspection_tool_feedback(
                        self._tool_tag_template
                    )
                    ...
                    for invalid_tool_call in tool_calls:
                        error_message = _build_inspection_protocol_tool_error(
                        ...
                        self._write_trace(
                            "\n----- 单表数据检索工具协议校验结果 -----\n"
                            f"tool_call_id: {invalid_tool_call.id}\n"
        ...
        if arguments is None:
            return TableDataInspectionResponse(
                status="failure",
                result="单表数据检索子智能体在允许次数内未提交有效工具调用。",
                raw_response=_serialize_model_responses(raw_responses),
                model_generation_count=len(raw_responses),
            )
        raw_response = _serialize_model_responses(raw_responses)
        ...
        try:
            ...
            rows, database_has_more = await self._query_page(base_sql, table_name, 0)
        ...
        if purpose == "entity_resolution" and not rows:
            ...
            if entity_config is not None and entity_config.match_mode == "name_fuzzy":
                ...
                try:
                    candidate_rows = await self._run_query_rows(
            ...
            if fallback_sql is not None and entity_config is not None and entity_config.match_mode == "name_fuzzy":
                try:
                    rows, database_has_more = await self._query_page(
                        fallback_sql,
                        table_name,
                        0,
                    )
    ...
    # 异步按缓存游标读取下一页；到达末页后不再访问数据库并返回明确提示。
    async def get_next_page(self, inspection_id: str) -> TableDataInspectionResponse:
    ...
    # 异步执行固定页大小查询并额外读取一行，用于判断是否还有下一页。
    async def _query_page(
        ...
        raw_rows = await self._run_query_rows(
    ...
    # 为候选数据读取设置硬超时，避免数据库故障永久占用查询任务。
    async def _run_query_rows(self, sql: str) -> list[dict[str, Any]]:
