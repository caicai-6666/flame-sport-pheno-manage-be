"""声明积分与奖品 Text-to-SQL 业务域及其资源边界。"""

from pathlib import Path

from app.agent.text2sql.domains.base import QueryDomainProfile, freeze_table_labels
from app.agent.text2sql.domains.rewards.planning_policy import (
...
REWARDS_QUERY_PROFILE = QueryDomainProfile(
    ...
    alignment_prompt_instructions=(
        ...
        "‘奖品’单独出现时也不能自行假设它表示商品目录、兑换记录或履约状态。"
        "这些含义会改变查询结果，必须根据上下文确定，无法确定时调用 ask_user。\n\n"
        "‘本赛季’或‘当前赛季’统一指当前进行中的赛季；‘正式参与’表示用户已锁定赛季要求数量的项目，"
        "并且已经选择挑战等级。对齐层只保留这些业务语义，不输出表名和字段名。"
    ),
    planning_prompt_instructions=(
        ...
        "计划必须为 point_record 声明当前流水和候选更晚流水两个角色：当前流水和候选集合都限定为有效，"
        "再用 none 量词排除更晚流水；predicate 限定候选有效，correlation_condition 同时按 user_id 关联，"
        "并限定候选 created_at 更晚，或 created_at 相同且 id 更大。"
        "查询待发放奖品时，必须同时确认这是有效的商品兑换记录，而不是商品目录，并让兑换类型、"
        "商品关联非空、流水有效和待发放四项条件同时成立；礼品履约状态只对商品兑换记录有业务意义。\n\n"
        "逐条积分流水或兑换履约必须返回 point_record.id，逐条赛季结算记录必须返回 season_user.id，"
        "逐条商品目录必须返回 product.id；关联表的其他 ID 不能替代查询主体的主键。\n\n"
