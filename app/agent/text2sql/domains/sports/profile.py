...
SPORTS_QUERY_PROFILE = QueryDomainProfile(
    ...
    alignment_prompt_instructions=(
        "### 7.1 凭证图片意图\n\n"
        "- 用户要求查看运动凭证、运动记录或打卡记录关联的图片时，"
        "必须保留“查看图片”的业务意图。\n"
        "- 禁止把该意图改写成“返回图片内容”或“返回图片地址”。\n"
        "- 对齐需求应表达为查看凭证关联图片，并保留每条凭证的唯一标识，"
        "供后续按标识查看图片。\n"
        "- 凭证标识是业务交付要求，不代表查询结果中存在图片内容字段。\n\n"
        "### 7.2 查询主体\n\n"
        "- 对齐需求必须明确查询主体。\n"
        "- 用户询问运动记录、运动凭证或打卡记录时，查询主体是逐条运动凭证明细。\n"
        "- 图片查看和凭证标识只是附加要求，不能替代逐条运动凭证明细这一主体。\n"
        "- 只有用户明确表示只需要凭证标识时，才将查询主体对齐为凭证标识。\n\n"
        "### 7.3 完成数量语义\n\n"
        "- “正式参与用户锁定 N 项且全部完成”中，N 表示赛季要求的锁定项目数量。\n"
        "- 对齐后的自然语言必须同时说明赛季要求锁定 N 项，以及用户全部完成其有效锁定项目。\n"
        "- 不得把赛季要求的锁定数量改写为用户‘恰好完成 N 项’。\n"
