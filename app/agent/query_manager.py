...
from app.agent.interaction.session import (
    AgentQueryCancelled,
...
class AgentQueryManager:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=self._max_active_sessions,
            thread_name_prefix="agent-query",
        )
    ...
    # 创建查询会话并启动后台线程执行同步模型工作流，立即向调用方返回 query_id。
    async def start_query(self, question: str, domain_key: str) -> AgentQuerySession:
        ...
        if self._settings.deepseek_api_key is None:
            raise RuntimeError("查询智能体尚未配置模型服务")
        profile = get_query_domain_profile(domain_key)
    ...
    # 在线程中执行现有同步 LangGraph 流程，确保模型和 asyncmy 包装不会阻塞 FastAPI 事件循环。
    ...
    # 应用退出时取消全部活动会话和异步包装任务，避免后台线程永久等待用户回答。
